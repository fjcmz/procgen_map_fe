...
- **City maps, character rosters, and the underground map are render-only zoom-in features** — they never affect the simulation and are never reached by the sweep harness.
...
| **[`claude_specs/underground_map.md`](claude_specs/underground_map.md)** | **Proposal (not yet implemented)**: alternate cavern/tunnel map view for rocky worlds. Lazy, render-only zoom-in toggle in the Generation tab — sits alongside the surface map at the same zoom level. Subtype-driven eligibility (gas giants never have one; volcanic/lava bias up to 0.60). |
...
Three layers run **on demand**, not in the worker pipeline:
...
- **Underground map** (`underground_map.md`, proposed) generates when the user toggles "View: Underground" in the Generation tab on a world with `mapData.hasUnderground === true`. The eligibility boolean is rolled in the worker via an isolated sub-stream; the cavern/tunnel graph itself is built on the main thread, cached via `useMemo` on the world seed.

All three run on the main thread, complete in milliseconds, and are seeded from `worldSeed` + a feature-specific suffix so re-opening the same entity produces a byte-identical artifact.
...
| `src/lib/underground/` *(proposed, not yet on disk)* | Underground map eligibility + lazy generator + renderer | `underground_map.md` |
...
Render-only flags (set on every generation, no payload weight):
- `hasUnderground?: boolean` *(proposed)* — true if this world is eligible for the underground map toggle. The cavern/tunnel data itself is NOT placed on `MapData`; it's built lazily on the main thread. See `underground_map.md`.
...
| `${seed}_underground_present` *(proposed)* | Worker-side eligibility roll: does this world have an underground map? (`underground_map.md`) |
| `${seed}_underground_largecaverns` / `_smallcaverns` / `_maze_<i>` / `_tunnels` / `_connections` *(proposed)* | Lazy main-thread underground generator slices — caverns, maze clusters, tunnel graph, surface entrances. Isolated so the sweep stays byte-identical. |
...
- **Underground map** (`src/lib/underground/`, proposed) — render-only, lazy. The worker-side `hasUnderground` boolean is rolled from an isolated sub-stream so the sweep must stay byte-identical when the feature lands.
...
- **Underground map is a render-only zoom-in.** *(Proposal — see `underground_map.md`.)* It does NOT generate cities, regions, resources, kingdoms, characters, or anything participating in the timeline. The worker only stamps a boolean `hasUnderground` on `MapData` (rolled from `${seed}_underground_present` so the sweep stays byte-identical); the actual cavern/tunnel graph is built on the main thread on first toggle, cached via `useMemo` on the world seed, and never crosses the worker boundary. Eligibility: gas giants → 0%, rocky/ice bodies → 30–60% based on subtype. Surface `cells` are read-only inside the underground generator — caverns may not mutate biome / elevation / `regionId`.
