...
- **City maps and character rosters are render-only zoom-in features** — they never affect the simulation and are never reached by the sweep harness. **Underground map geometry** is render-only too, generated eagerly in the worker so the cavern/tunnel graph ships with `MapData`. **Underground resources, however, are sim-integrated**: cavern deposits project onto surface regions and feed trade / tech-discovery / war.
...
**Underground maps** (`underground_map.md`) are generated **eagerly** inside `mapgen.worker.ts` **before** the history pipeline (so cavern-resource attachment happens during `buildPhysicalWorld`). Geometry is render-only — caverns, tunnels, and connections never participate in the timeline. Resources are sim-integrated: every cavern's deposits project onto the nearest surface region, ride the standard tech-discovery + trade pipeline, and feed trade / war / wealth metrics. Eligibility is rolled via an isolated sub-stream `${seed}_underground_present`; seeds that fail the roll consume zero additional underground sub-streams and stay byte-identical to the pre-feature sweep.
...
| `${seed}_underground_largecaverns` / `_smallcaverns` / `_maze_top` / `_maze_<i>` / `_tunnels` / `_connections` | Worker-side underground generator slices — cavern BFS-grow, maze cluster placement + internal A* passages, inter-cavern A* tunnel graph, surface entrances. Isolated so seeds without an underground stay byte-identical. |
| `${seed}_underground_resources` | Per-cavern resource rolls in `attachUndergroundResources` (`history.ts`). Underground deposits project onto surface regions and ride the existing tech-discovery / trade machinery — sim-integrated layer of the underground feature. |
...
- **Underground map geometry** (`src/lib/underground/`) — render-only. Worker generates it eagerly via isolated `${seed}_underground_*` sub-streams. **Underground resources** (`attachUndergroundResources` in `history.ts`) are sim-integrated through `${seed}_underground_resources`, project onto surface regions, and ARE covered by the sweep harness — `scripts/sweep-worker.ts` rolls the `_underground_present` gate and passes the `UndergroundMap` into `historyGenerator.generate`. Seeds without an underground stay byte-identical to the pre-feature baseline; seeds with one shift trade / war / wealth metrics in `HistoryStats` deliberately.
...
- **Underground map: geometry is render-only, resources are sim-integrated.** See `underground_map.md`. Cities, kingdoms, characters never live in caverns. **Resources do** — each cavern's deposits project onto the nearest surface region's `region.resources` so countries with overlapping territory pick them up through the standard tech-discovery / trade pipeline. The worker rolls eligibility on `${seed}_underground_present` BEFORE the history pipeline, generates the cavern/tunnel graph via `generateUnderground(...)`, then threads the `UndergroundMap` into `historyGenerator.generate` so `attachUndergroundResources` runs inside `buildPhysicalWorld` (after surface resources land, before year-0 discovery). The underground builds its **own Voronoi graph via `buildCellGraph(\`${seed}_underground_graph\`, surfaceCells.length, ...)`** — same cell count as the surface map (so polygon density matches), independent layout. Cells are classified `solid` / `cavern` / `tunnel`; caverns are BFS-grown polygon groups; tunnels are single-cell-wide chains found by A* (binary min-heap). Eligibility: gas giants → 0%, rocky/ice bodies → 30–60% based on subtype. Surface `cells.biome` / `.elevation` / `.regionId` are read-only inside the underground generator. Every random draw routes through `${seed}_underground_*` sub-streams — seeds without an underground stay byte-identical to the pre-feature sweep. The `GENERATE_HISTORY` path doesn't re-emit underground geometry; `App.tsx` forwards the previous `mapData.underground` as `previousUnderground` so the history-only run still attaches cavern resources to regions and the toggle survives.
