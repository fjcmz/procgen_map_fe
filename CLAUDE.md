...
- `historyStats?` — `HistoryStats` aggregate (Phase 3): peak population, totals per event type, plus `totalTechs` and `peakTechLevelByField` for tech introspection. Forwarded straight from `historyGenerator.generate()`; do not recompute on the main thread.
...
- `acquiredTechs` — `{field, level}[]` delta the conqueror gained, populated on CONQUEST events only when non-empty
- TECH event descriptions are now formatted by `buildTechDescription()` in `HistoryGenerator.ts` as `"{Country} discovers {field} level N (by a {illustrateType} in {city})"` with graceful fallbacks for stateless techs
