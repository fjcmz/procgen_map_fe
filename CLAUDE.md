...
| `src/components/UnifiedOverlay.tsx` | Tabbed draggable overlay shell (see `specs/overlays_tabs.md`). Hosts the Generation tab today; Events/Hierarchy/Tech tabs are Phase 2–4 placeholders (`<div>Coming soon</div>`) and are grayed out until `mapData.history` exists. Collapse state, active tab, and the `OVERLAY_WIDTHS: Record<OverlayTab, number>` per-tab width table live here. Default position `top: 16, right: 16`. Drag handle is the title row + tab bar; when collapsed (▴), only the title row stays visible (tab bar and body both hide) |
| `src/components/overlay/GenerationTab.tsx` | Generation-tab body: seed input, cell count, water ratio slider, terrain/political view toggle, season selector, layer toggles, history toggle + sim-years slider, generate button + progress bar. Extracted verbatim from the old `Controls.tsx` — same props, same form styles; only the panel chrome (fixed positioning, border, title bar, collapse toggle) moved up into `UnifiedOverlay.tsx` |
| `src/components/Timeline.tsx` | Two independent draggable panels: (1) bottom timeline controls (play/pause, step ±1/±10, year slider, population/nation stats), (2) right-side event log (collapsible via ▴/▾ toggle, yearly population entries). Rendered only when `mapData.history` exists. **Spec stretch §5**: event log panel also hosts a collapsible "Tech" sub-panel (positioned between `logHeader` and `logList` — NOT inside the scrollable `logList`, since `logEndRef.scrollIntoView` would push it off screen on every play tick) with a 9-swatch legend and a 240×80 canvas drawing one polyline per tech field from `historyData.techTimeline.byField`, plus a vertical cursor at `selectedYear`. `TECH_FIELD_COLORS: Record<TechField, string>` is exhaustive over the union so adding a new tech field becomes a compile-time error |
...
- **Unified overlay** (`UnifiedOverlay.tsx` + `overlay/GenerationTab.tsx`): single draggable, collapsible top-right panel that will eventually host four tabs — Generation, Events, Hierarchy (Realm), and Tech — per `specs/overlays_tabs.md`. Phase 1 lands the shell and populates only the Generation tab; the other three tabs are grayed-out placeholders (`Coming soon`) that enable once `mapData.history` exists (planned for Phase 2–4). Drag handle is the title row + tab bar. Collapse toggle (▴/▾) in the title row hides both the tab bar and the active-tab body, leaving only the title row visible — matches the old `Controls.tsx` collapse UX. `activeTab` and `collapsed` are local `useState`. The `GenerationTab` body contains: seed input, detail/cell-count buttons, water ratio slider, "Generate History" checkbox + conditional "Sim years" slider, terrain/political view toggle (two buttons that set `mapView`), season selector (four buttons: Spring/Summer/Autumn/Winter — a render-time-only control that shifts polar biome boundaries and permafrost overlay intensity without re-running generation), layer checkboxes, generate button, and progress bar.
...
- **Timeline panel** (`Timeline.tsx`): rendered only when `mapData.history` exists. Two independent draggable panels:
  ...
  - **Tech sub-panel** (inside the event log panel, above the scrollable event list): a collapsible ▴/▾ "Tech" section with a 9-swatch legend and a 240×80 canvas rendering one polyline per tech field from `historyData.techTimeline.byField`. A vertical cursor marks `selectedYear` and redraws in the same `useEffect` as the polylines (a single-canvas design — the chart is small enough that a full redraw on every year change is sub-millisecond, so the two-canvas split the spec hints at is unnecessary). Y-axis auto-scales to the running-max peak with a floor of 1 to handle empty histories. The sub-panel sits outside the `logList` scroll container so `logEndRef.scrollIntoView` doesn't scroll it off-screen on play ticks.
