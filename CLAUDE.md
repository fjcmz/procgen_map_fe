...
```
voronoi → elevation → oceanCurrents → moisture → temperature → biomes → rivers → buildPhysicalWorld
...
| `src/lib/renderer/renderer.ts` | Canvas 2D rendering — all layers, biome fill, hillshading, ocean current-tinted water depth, borders, icons |
