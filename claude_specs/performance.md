...
| **Universe** (`src/lib/universe/`) | Uninstrumented. Physics generation ~1.7s at 10,000 systems; the optional history sim is ~1.2s at 10,000 systems / 5000 steps after the 2026-06 perf work (was ~51s: per-step life rolls were replaced by geometric sampling into an event schedule, and civ reach/candidate scans by incremental held-system bookkeeping + per-galaxy `allBodies` ranges — see `universe_map.md` pitfalls). Render hot path is `UniverseCanvas` zoom/pan; per-color glow canvas cache + frustum culling already exist. | Profile in DevTools Performance panel. Don't reintroduce O(bodies × steps) passes in `UniverseHistoryGenerator` — schedule events or maintain incremental collections instead. The canvas renderer benefits more from culling refinements than algorithmic changes to generation. |
