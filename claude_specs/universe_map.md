...
| `UniverseHistoryGenerator.ts` | Optional `numSteps`-step timeline simulation (each step = 1 million years). For every habitable body, samples the 0.005%/step life-spawn chance on `${seed}_universe_life_${bodyId}` and the 0.07%/step advancement chance on `${seed}_lifeevolution_${bodyId}` — both via **geometric sampling** into a per-step event schedule (one draw per event, not one per step; see "Universe History" below). Emits `LIFE_APPEARED` (always unicellular) + `LIFE_ADVANCED` events and mutates the body's `life` / `lifeLevel` / `biome` / `subtype` so the serializer downstream sees end-of-time values |
...
Life always starts at `unicellular`. Each subsequent stage advances with probability 0.07% per step (`LIFE_ADVANCE_CHANCE_PER_STEP`).

**Geometric sampling, not per-step rolls.** Both probabilities are implemented by sampling the step of the next success directly (`stepsUntilSuccess(u, p) = floor(log(1-u) / log(1-p))`) and parking the body in a per-step event schedule until then — distribution-identical to rolling every step, but O(events) instead of O(bodies × steps). The old per-step roll loop was the dominant cost of the whole simulation (~880M draws at 10,000 systems); switching to sampling changed how the per-body sub-streams are consumed, so same-seed histories shifted once when this landed (2026-06; deliberate — the universe layer has no sweep coverage). A `scheduleToken` per body invalidates a pending natural-appearance event when a terraform completes on the body first.

**Two PRNG sub-streams per body** — kept separate so adding the evolution layer didn't shift the earlier spawn timings:

| Sub-stream | Purpose |
|------------|---------|
| `${seed}_universe_life_${bodyId}` | First-appearance sample (0.005%/step equivalent) **and** the biome / subtype pick on success |
| `${seed}_lifeevolution_${bodyId}` | Advancement samples (0.07%/step equivalent) once life exists and the body is below `intelligent_animals` |
...
Events are emitted in non-decreasing step order by construction (the outer loop is per-step), so no final sort is needed; within a step, life events fire in schedule order, then founding / expansion / terraform-completion events in pass order.
...
| `expansion.ts` | Reach algorithm + target-selection: held-galaxy systems plus wormhole-partner-galaxy systems, weighted inverse-distance with in-galaxy bias 1.0 / cross-galaxy 0.4. Reach reads the per-civ `heldSystemIds` set maintained incrementally by `CivilisationGenerator` (no occupancy scans); candidate collection walks only the reachable galaxies' contiguous `[start, end)` ranges of `allBodies` (`galaxyBodyRanges`), which relies on galaxies being sequential chunks of the system array |

**Loop structure in `UniverseHistoryGenerator`**: outer loop is per-step; inner passes are (1) life events due this step (from the geometric-sampling schedule), (2) civ-founding rolls over the small `intelligentPending` list (orderIdx-sorted — bodies at `intelligent_animals` without a civ), (3) per-civ expansion, (4) terraform completion (bucketed by `completeStep`). Nothing iterates all habitable bodies per step anymore. Occupancy state is mirrored into `CivContext.ownerByBody` (running current-occupant map) and per-civ `systemHoldCounts` / `heldSystemIds` by `claimBody`, which every owner-setting occupancy write must go through.
...
- **Life evolution rolls use TWO isolated sub-streams per body.** `${seed}_universe_life_${bodyId}` is the spawn sample (also feeds biome + subtype on success); `${seed}_lifeevolution_${bodyId}` is the advancement sample. Keep them separate — collapsing them would shift spawn timings for every pre-existing seed. If you add a third life-related roll, give it its own sub-stream too. Both streams are consumed via geometric sampling (one draw per event); converting any of them back to per-step rolls — or vice versa — changes same-seed histories even though the distribution is identical.
...
- **Universe-history loop is per-step outer, event-driven inner.** Life events come from the geometric-sampling schedule; the civ passes run per step but only touch small collections (`intelligentPending`, `civRuntimes`, the `completeStep` terraform bucket). Do NOT add a new pass that iterates all bodies (or all habitable bodies) every step — that O(bodies × steps) shape is exactly what made history generation take ~50s at 10,000 systems before the 2026-06 perf work (now ~1.2s). If you add another per-step behaviour, either schedule it (geometric sampling for "first success at step S" rolls) or key it off a small incrementally-maintained collection, and give it its own sub-stream.
- **Every owner-setting occupancy write must go through `claimBody`.** `CivContext.ownerByBody` and the per-civ `systemHoldCounts` / `heldSystemIds` are incremental mirrors of `occupancyByBody` — candidate predicates and reach read ONLY the mirrors, never the entry arrays. If you `appendOccupancy` an OUTPOST / COLONY / TERRAFORM_COMPLETE entry without the matching `claimBody` call, reach and candidate filtering silently diverge from the recorded history. TERRAFORM_START deliberately does NOT claim (the body stays unclaimed while work is in progress — and can be outposted by another civ in that window, which is why the bookkeeping uses per-system counts, not a set).
- **Candidate scans rely on galaxies being contiguous chunks of `solarSystems`.** `galaxyBodyRanges` maps each galaxy to a `[start, end)` range of `allBodies`. If `UniverseGenerator`'s grouping ever stops being sequential chunking, rebuild that index as per-galaxy body lists instead — iterated in canonical galaxy order, or candidate ordering (and thus target picks) changes.
