...
| `UniverseHistoryGenerator.ts` | Optional `numSteps`-step timeline simulation (each step = 1 million years). For every habitable body, rolls the existing 0.005%/step spawn chance for life on `${seed}_universe_life_${bodyId}` and the new 0.1%/step advancement chance on `${seed}_lifeevolution_${bodyId}`. Emits `LIFE_APPEARED` (always unicellular) + `LIFE_ADVANCED` events and mutates the body's `life` / `lifeLevel` / `biome` / `subtype` so the serializer downstream sees end-of-time values |
...
Life always starts at `unicellular`. Each subsequent stage requires a successful 0.1%/step roll (`LIFE_ADVANCE_CHANCE_PER_STEP`).
...
| `${seed}_lifeevolution_${bodyId}` | Per-step advancement roll (0.1%) once life exists and the body is below `intelligent_animals` |
