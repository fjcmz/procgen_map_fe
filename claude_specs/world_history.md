...
- **Ownership serialization is an incremental replay with two load-bearing invariants.** The per-year ownership arrays and expansion flags in `HistoryGenerator.generate` are built from running replay state (`regionOwner` / `expansionOwner` / chronological expansion list / `expansionFlagOwner`) advanced once per serialized year, plus an O(cells) fill — NOT a full timeline replay per year. Exactness rests on: (A) a region hosts at most one country's `governingRegion` ever (country formation requires an unowned region with all cities alive; dissolution leaves only ruins, so the predicate can never re-pass), and (B) expansion events have pairwise-disjoint cell sets (a region is expanded at most once). A future feature that re-forms countries on dissolved regions or re-expands regions breaks this — revisit the replay in that case. Seeding must still include both `world.mapCountries` and `world.mapDeadCountries` (dead countries keep a valid `governingRegion`, and conquest chains referencing them must transfer their regions).
- **All `ownedCells` mutations go through `claims.ts::claimCell`.** It maintains three derived indexes incrementally: `city.cellCapSum` (growth-step carrying capacity), `city.ownedCellRegionIds`, and the refcounted `world.usableClaimRefs` (the claimed-cells index read by YearGenerator steps 4c / 4c-sea). Every `mapUsableCities` entry/exit must call `registerUsableCityClaims` / `releaseUsableCityClaims` (the release also bumps `world.regionClaimEpoch`, which invalidates per-city frontier-exhausted caches). A direct `ownedCells.set` or an unpaired usable-map mutation silently desyncs these indexes and shifts the sweep. Exception: `Foundation.ts`'s set is a guaranteed overwrite of the founding-cell key pre-seeded in `buildPhysicalWorld`. CitySettlement's own `claimedCells` map is still rebuilt per year on purpose — its VALUES feed `isTooCloseToOtherCity` and depend on `mapUsableCities` iteration order, which a persistent index cannot reproduce.
- **`numSimYears` truncates the simulation itself.** `TimelineGenerator.generate` runs exactly `numSimYears` years (clamped 1–5000) — not 5000 with serialization-time truncation. The year loop is strictly forward, so a truncated run's years are byte-identical to the prefix of a full run; the sweep always passes 5000.
