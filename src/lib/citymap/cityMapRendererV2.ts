...
function drawRuinOvergrowth(
  ...
  // Mountain polygons are also excluded from the overgrowth pass so the
  // stone range doesn't silently swap to reclaim green on ruin cities.
  const mountainIds = new Set<number>(data.mountainPolygonIds ?? []);
  ...
  for (const polygon of data.polygons) {
    ...
    if (mountainIds.has(polygon.id)) continue;
