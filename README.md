...
- **Parametric terrain profiles** — infrastructure for preset planet profiles (desert, ice, forest, swamp, mountain, ocean) that override ~40 tunable terrain constants; a `TerrainProfile` interface threads through every terrain function so future phases can add a UI selector and per-profile presets without further pipeline changes
