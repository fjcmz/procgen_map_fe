...
- **Rich terrain** — 20 biome types classified via a Whittaker diagram (elevation × moisture), including a transitional alpine meadow band and a dedicated inland **lake** biome for depression-fill outflows; per-cell vegetation density variation modulates biome fill colors and tree icon density/size based on each cell's position within its moisture band, creating organic gradients within biome regions
...
- **Hydrology** — rivers generated from drainage accumulation; hydraulic erosion carves visible river valleys, gorges, and floodplains into the terrain using a stream power model (erosion proportional to flow and slope), with valley widening for hillshading visibility. A Priority-Flood + ε depression-fill pass before every river trace guarantees rivers always reach the ocean or terminate at an inland lake — small closed basins are materialized as visible freshwater lakes (the 20th biome), and larger basins keep their land biomes but are routed through via a virtual drainage surface. River rendering is zoom-aware: at the default fit-to-screen view only the largest trunk rivers are drawn with thin strokes, and as you zoom in progressively smaller tributaries become visible while stroke widths stay at a consistent screen-pixel thickness regardless of zoom level
...
7. **Depression fill (pass 1)** — a Priority-Flood + ε pass produces a parallel `drainageElevation` surface so every land cell has a strictly-lower neighbour toward the ocean; closed basins whose size falls inside the `[lakeMinSize, lakeMaxSize]` window (default `[4, 20]`, overridden to `[2, 80]` for `forest` and `[2, 150]` for `swamp`) are materialized as visible LAKE cells. Smaller "micropit" basins are filtered out as noise; larger basins stay as land and drain virtually across the filled surface
8. **Rivers (initial)** — water flow accumulation determines river paths and per-cell `riverFlow` values, using the depression-filled drainage so no river dead-ends mid-land
9. **Hydraulic erosion** — stream power erosion carves valleys along river paths (erosion ~ flow^0.5 × slope), with sediment deposition creating floodplains downstream and valley widening for visible cross-sections in hillshading; erosion intentionally uses raw `cell.elevation` since its deposition step breaks monotonicity
10. **Depression fill (pass 2)** — erosion can create new sinks, so `fillDepressions` runs again on the eroded terrain before the final river trace; any new small basins become additional lakes
11. **Rivers (final)** — re-traced on the carved terrain; temperature and biomes are refreshed afterward since elevation changed (the `isLake` short-circuit in `assignBiomes` protects LAKE cells from being overwritten, but runs after the polar ICE check so cold lakes still freeze)
12. **Physical world** — always runs: BFS flood-fills connected land cells to detect continents (lake cells are naturally excluded since they are `isWater`); subdivides each continent into geographic regions (~30 cells each) via multi-source BFS seeding; places 1–10 natural resources per region (weighted random type across 17 resource types); places 1–5 cities per region using climate-aware scoring (river mouths, natural harbors, biome penalties with river/coast mitigation)
13. **History** *(optional)* — if enabled, the **HistoryGenerator** orchestrates a full civilizational simulation: it first builds the physical world (step 12), then runs a 5000-year timeline via the **TimelineGenerator**. Each year, 12 event generators fire in order: cities are founded, make first contact, form countries, produce illustrious figures, discover technologies, build wonders, found religions, open trade routes, suffer cataclysms, wage wars, resolve conquests, and form empires. The first N years (user-configurable) are serialized into the UI's timeline format with ownership snapshots for fast scrubbing.
14. **Roads** *(history only)* — A* pathfinding connects history-generated cities across the terrain
15. **Trade routes** *(history only)* — active trade connections are pathfound using a dual-domain A* that traverses both land and water; maritime segments use a cost gradient based on distance from land, naturally producing routes that hug coastlines and hop between islands rather than cutting straight across open ocean. Per-city trade-route capacity scales with the `exploration`, `growth`, `industry`, and `government` tech levels (each known tech multiplies the size-based base cap by `1 + level/10`), matching the spec in `specs/04_City.md`

If history is **disabled**, steps 13–15 are skipped and the map shows terrain and physical world structure only (no kingdom simulation, roads, or timeline).
...
│   │   ├── depressionFill.ts   # Priority-Flood + ε — no-sink drainage surface + LAKE materialization
