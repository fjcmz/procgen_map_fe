...
7. **Depression fill (pass 1)** — a Priority-Flood + ε pass produces a parallel `drainageElevation` surface so every land cell has a strictly-lower neighbour toward the ocean; small closed basins (≤ `lakeMaxSize` cells, default 60) are materialized as visible LAKE cells, while larger basins stay as land and drain virtually across the filled surface
